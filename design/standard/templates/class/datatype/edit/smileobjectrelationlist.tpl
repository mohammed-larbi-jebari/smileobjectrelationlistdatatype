...
<select id="eccaorl_selection_{$class_attribute.id}" name="ContentClass_smileobjectrelationlist_selection_type_{$class_attribute.id}">
...
<div class="block">
    {if eq( ezini( 'BackwardCompatibilitySettings', 'AdvancedObjectRelationList' ), 'enabled' )}
        ...
        <select id="eccaorl_type_{$class_attribute.id}" name="ContentClass_smileobjectrelationlist_type_{$class_attribute.id}">
    ...
    {else}
        <input type="hidden" name="ContentClass_smileobjectrelationlist_type_{$class_attribute.id}" value="2" />    
    {/if}
</div>

<div class="block">
    <label for="eccaorl_allowed_{$class_attribute.id}">{'Allowed classes'|i18n( 'design/standard/class/datatype' )}:</label>
    <select id="eccaorl_allowed_{$class_attribute.id}" name="ContentClass_smileobjectrelationlist_class_list_{$class_attribute.id}[]" multiple="multiple" title="{'Select which classes user can create'|i18n( 'design/standard/class/datatype' )}">
...
<input type="hidden" name="ContentClass_smileobjectrelationlist_placement_{$class_attribute.id}" value="{$default_placement.node_id}" />
{section-else}
<p>{'New objects will not be placed in the content tree.'|i18n( 'design/standard/class/datatype' )}</p>
<input type="hidden" name="ContentClass_smileobjectrelationlist_placement_{$class_attribute.id}" value="" />
{/section}
